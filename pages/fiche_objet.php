...
<body>
    ...
    <?php if (isOwner($idObjet , $_SESSION['id_membre'])) { ?>
        <form action="../traitement/traitement_image.php?operation=1&&id_objet=<?=$idObjet?>" method="POST" enctype="multipart/form-data">
    ...
    <div class="container">
        <?php while ($donnee = mysqli_fetch_assoc($img)) { ?>
            <img src="../traitement/uploads/<?= $donnee['nom_image'] ?>" alt="">
                <?php if(isOwner($idObjet , $_SESSION['id_membre'])) ?>
                    <a href="../traitement/traitement_image.php?operation=0&&id_image=<?=$donnee['id_image']?>"></a>
                <?php ?>
            <?php } ?>
    </div>
